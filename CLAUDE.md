...
**Статистика:** 1835 тестов в репозитории (api 812 + web 640 + mobile 255 + shared 85 + auth 36 + db 7).
...
19. **Theme tokens — single source of truth (B3-THEME, ADR 0009).** _(номер #19 — следующий за #15; #16-18 заняты evergreen-патернами в §9)._ Web поддерживает обе темы (light default + dark) через CSS-variables на `:root, .theme-light` (admin globals) и `[data-marketing="true"]` (marketing scope) + `.theme-dark` overrides. Tailwind v4 `@theme inline { --color-X: var(--X); }` публикует tokens как utilities (`bg-layer-0`, `text-text-primary`). **НЕ** используем Tailwind `dark:` modifiers, **НЕ** пишем hardcoded HEX в компонентах (исключения: brand orange `#F97B10` — same в обеих темах, `--brand-foreground: #0a0a0b` для text-on-brand-button — fixed near-black, AA pass на orange). Theme-switch — single class change на `<html>` через `setMode()` (provider) или inline `<ThemeScript />` (FOUC prevention в `<head>`). Persistence: localStorage `jumix-theme-mode` (anonymous) + `users.theme_mode` column (logged-in, sync на login через `useThemeSync` hook). `PATCH /me/preferences` endpoint (own user only). MapLibre theme через `getMapStyleFor(theme)` + `useMapStyleEpoch(map)` hook (counter инкрементируется на `style.load`, layer-компоненты добавляют его в deps чтобы re-add sources/layers после `setStyle()`). SVG mockups в landing — CSS classes inside `<defs><style>` reading `--m-mockup-*` tokens (SVG attrs не принимают `var()`). Toggle — single button (light↔dark, без 'system' опции в UI; mode preserved в model). Mobile НЕ trogaем (StyleSheet tokens dark-hardcoded — отдельная вертикаль M-THEME в backlog). Детали — ADR [0009](docs/architecture/adr/0009-theme-architecture.md).
...
### B3-THEME — Light theme + toggle (April 2026)
Web поддерживает light + dark темы (default — light, заказчик preference; OS `prefers-color-scheme` уважается на first visit). 4 slices + hotfix + toggle simplification. **Foundation (B3-THEME-1):** CSS-tokens рестракчурен — `:root, .theme-light` (default) + `.theme-dark` (override), Tailwind v4 `@theme inline` публикует utilities. ThemeProvider + useTheme hook + ThemeScript inline (FOUC prevention в `<head>`) + localStorage persist + DB sync (миграция 0014 `users.theme_mode`, `PATCH /me/preferences` endpoint, `useThemeSync` hook anti-loop через `lastPushedMode` ref). +10 backend tests + 32 web tests. **Admin (B3-THEME-2):** ThemeToggle в Topbar; MapLibre theme switching (`LIGHT_VECTOR_STYLE` protomaps light flavor, `getMapStyleFor`, `useMapStyleEpoch` hook — layer-компоненты pере-регистрируют source+layer после `map.setStyle()`); ShiftPathLayer endpoint stroke theme-aware (dark→near-black, light→white); Avatar fallback color #71717a (neutral, читается на обеих). Admin cabinet уже был token-based из B3-UI-1, так что hardcoded zinc-* не было — slice короче плана. Recharts не используется (analytics в Этапе 3, ждёт спеку начислений). **Marketing (B3-THEME-3):** marketing.css `--m-*` palette стала theme-aware (light variant — generous whites + cream surface `#fcfcfb`/`#ffffff`, dark preserved); MarketingThemeToggle в landing-header (использует `--m-*` palette); SVG mockups (3 illustrations) refactor через CSS classes в inline `<defs><style>` reading `--m-mockup-*` sub-palette — SVG fill/stroke attrs не принимают `var()`, нужен CSS-class layer; feature-card danger tone из `bg-red-500/10 text-red-400` → `--color-danger` (theme-aware semantic); survey-flow errors `#ef4444` → `var(--color-danger)`. **Hotfix:** изначально light в marketing.css был на `:root [data-marketing], .theme-light [data-marketing]` — равная specificity с `.theme-dark`, source-order побеждал → light всегда выигрывал dark. Fix: убрать :root + перенести defaults на голый `[data-marketing]` (0,1,0), dark на class-prefixed (0,2,0). **Toggle simplification:** заказчик upreked dropdown с 3 опциями как лишний UX — теперь обычный button-toggle (light↔dark, иконка показывает target состояние: Moon в light → click→dark, Sun в dark → click→light). 'system' опция убрана из UI, но preserved в `ThemeMode` model. **Polish (B3-THEME-4):** добавлен `--brand-foreground: #0a0a0b` token (always near-black, AA pass на orange) — admin primary button и create-site step-indicator переехали с `text-layer-0`/`text-white` на `text-brand-foreground` для WCAG AA в light theme. ADR 0009. Mobile (Expo) — НЕ trogaем (StyleSheet tokens dark-hardcoded, отдельная вертикаль M-THEME в backlog). → [ARCHITECTURE.md#b3-theme](ARCHITECTURE.md#b3-theme-light-theme--toggle)

### B3-SURVEY — Customer development surveys (April 2026)
4 customer development surveys (b2b-ru, b2b-kk, b2c-ru, b2c-kk) для capture insights от потенциальных клиентов перед mass marketing push. **Schema:** 3 tables — `surveys` (templates, seeded, slug-keyed), `survey_questions` (FK к surveys, position-ordered с UNIQUE per survey, `group_key`/`group_title` для UI sections), `survey_responses` (public submissions, JSONB `answers` keyed by position-as-string, denormalized contact `full_name`/`phone`/`email` для leads). `honeypot_filled` column для silent bot rejection (filled → stored с marker для analytics, returns 200 OK к bot'у). Migration 0013. **Public submission:** `POST /api/v1/surveys/:slug/responses`, no auth, rate-limited via `@fastify/rate-limit` (3 per IP per 24h, errorResponseBuilder возвращает `AppError` чтобы попасть в общий error-handler). Honeypot field name `website_url` hidden via CSS `position: absolute; left: -9999px` + `aria-hidden` + `tabindex=-1` + `autocomplete=off`. Validation: zod schema, phone `+7XXXXXXXXXX` format, email RFC, all required answers. **Admin endpoints:** `/api/v1/admin/surveys` list с aggregated counts (LEFT JOIN + GROUP BY на 4 templates — простая, не масштабируется до 1000s но не нужно), `/admin/surveys/:slug/responses` paginated с search (JSONB `::text ILIKE`)/dateRange/spam filters, `/admin/surveys/:slug/responses/:id` full detail с questions joined. **Public UX (`/survey/[slug]`):** server component fetches survey + questions, client `SurveyFlow` state machine (intro → questions one-per-screen → contact → success → auto-redirect к landing после 5s); AnimatePresence transitions с premium easing `cubic-bezier(0.22, 1, 0.36, 1)`; progress bar (group + question position); back button preserves answers; `beforeunload` warning при unsaved content. **Locale-aware routing на landing** — `useLocale()` picks `b2b-ru`/`b2c-ru` для `ru` или `b2b-kk`/`b2c-kk` для `kz`. (Note: marketing-locale store использует `'kz'`, backend slugs используют ISO `'kk'` — synonyms.) **Admin UX:** `/surveys` (superadmin only) — list page с 4 template-cards (response counts, latest timestamp, active toggle); `/surveys/[slug]` — DataTable + filters (search, spam toggle) + drawer с full Q&A grouped по `groupKey`. Read-only — нет edit/delete для data integrity. **Seed:** `packages/db/scripts/seed-surveys.ts` (idempotent UPSERT by slug; questions DELETE+INSERT для clean text updates) seeds 4 surveys + 42 questions total (11+11+10+10) — content extracted из castdev WhatsApp-version docx files (без interviewer hints, group structure preserved). Tests +86 (api +31, web +55). → [ARCHITECTURE.md#b3-survey](ARCHITECTURE.md#b3-survey-customer-development-surveys)
...
- B3-SURVEY — customer development surveys → [ARCHITECTURE.md#b3-survey](ARCHITECTURE.md#b3-survey-customer-development-surveys)
- B3-THEME — light theme + toggle → [ARCHITECTURE.md#b3-theme](ARCHITECTURE.md#b3-theme-light-theme--toggle)
