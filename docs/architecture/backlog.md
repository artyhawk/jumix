...
`operators.availability` (`free | busy | on_shift | null`) сейчас read-only — имеет CHECK что `NULL ⇔ status≠active`, но НЕТ endpoint'а для изменения. Это сознательно: availability меняется не admin-fiat, а как функция shifts state.

**Update (M4):** после shifts-модуля (ADR 0006) availability СТАНОВИТСЯ derived state от `shifts.status`. На MVP мы её не обновляем автоматически — canWork + «есть ли active shift» уже покрывают UI need. Если заказчик попросит явный «free/busy/on_shift» индикатор — shift service должен писать availability в транзакции. Endpoint ручного override — скорее всего НЕ нужен (shifts теперь source of truth).
...
### Scheduled shifts (plan in advance)

Сейчас state machine — `null → active → paused → ended`. Нет `scheduled` состояния. Owner/superadmin не могут запланировать «смену завтра в 8:00 на кране X для оператора Y». Planning UI — отдельная вертикаль (shift-planner + calendar view). Backlog до появления заказчика-триггера (сейчас крановщики сами решают когда выходить).

### Shift cancellation (soft-delete with reason)

Ошибочный старт (нажал «Начать» вместо «Отмена» в ActionSheet) — сейчас оператор обязан end с duration ~10 секунд. Мусор в истории.

План: `POST /shifts/:id/cancel` с required reason, доступен только оператору-владельцу в первые 5 минут после старта (чтобы не отменять реальную проделанную работу задним числом). Добавить `cancelled` в state-machine + soft-delete column, либо пометить terminal status.

### Offline shift queue (M5 dependency)

На стройке может не быть сети. Оператор не должен зависеть от connectivity чтобы start/end. План:
- Mutation queue persisted через MMKV / AsyncStorage
- Retry on connectivity resumed
- Conflict resolution (clock skew → server now() authoritative на create, client-provided startedAt — metadata only)

Завязано на M5 offline patterns для GPS pings — будет общая архитектура.

### Shift notes UI (detail screen edit)

Backend поле `shifts.notes` работает (принимается на end), но UI для editing/viewing на mobile минимальный. Backlog: редактируемое поле на detail screen + explicit «Add notes» CTA в ActiveShiftCard. Пока оператор даже не видит notes если их установил.

### Location playback (M5 GPS visualization)

Когда M5 добавит GPS pings к shift'ам, detail screen должен показывать track: polyline на MapLibre + time-scrubber. Web site drawer — heatmap density. Offload до M5 закрытия.

### Backend-enriched crane list с currentShift

`/my-cranes` на web сейчас делает client-side merge (fetch cranes + fetch shifts, match in JS). План — backend enricher: `cranes.list` возвращает `currentShift: {operatorName, startedAt} | null`. Устраняет N+1 на клиенте. Триггер — когда список станет >50 cranes per page.

### Force-end shift (superadmin override)

Сейчас только operator-владелец может end shift (rule #shiftPolicy.canChangeStatus). Если operator потерял телефон / экстренно ушёл — shift может «висеть» active бесконечно. Backlog: `POST /shifts/:id/force-end` для superadmin с required reason → audit trail + notification оператору (когда push сделаем в M7).

### Pause soft time-limit warning

Advisory pause удобен, но 8-часовая пауза очевидно bug. Backlog: cron (M4+) scan'ящий active shifts > 16h старта или paused > 4h → notification owner'у «проверьте оператора X».

### Shift creation для других пользователей

Сейчас только operator self (ctx.userId → shift.operatorId). Owner «назначил крановщика на смену» — backlog, нужен UI + audit delineation («кто начал смену — сам оператор или owner от его имени»).
